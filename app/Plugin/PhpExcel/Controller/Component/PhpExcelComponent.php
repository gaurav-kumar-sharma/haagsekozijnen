...
class PhpExcelComponent extends Component {
    ...
    public function addTableHeader($data, $params = array()) {
        ...
        $this->_xls->getActiveSheet()->getStyle($this->_row)->getFill()->applyFromArray(array(
        ...
        //$this->_xls->getActiveSheet()->getStyle($this->_row)->applyFromArray($styleArray);
        $offset = 0;
     ...
     */
    public function output($filename, $writer = 'Excel2007') {  
        $objWriter = $this->getWriter($writer);
        $objWriter->save('files/ExcelFiles/' . $filename);
       ...
       public function output_to_file($filename, $writer = 'Excel2007') {  
        $objWriter = $this->getWriter($writer);
        $objWriter->save('files/ExcelFiles/' . $filename);
        ob_end_clean(); 
        return true;
